...
func _ready():
	...
	quadrosDavid = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("David1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("David2"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("David3")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosDavid[quadro-1].position.x = 1058.391
	quadrosDavid[quadro-1].position.y = -2051.065
	quadrosDavid[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosDavid[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosDavid[quadro].position.x = 1058.391
		quadrosDavid[quadro].position.y = -2051.065
		quadrosDavid[quadro+2].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosDavid[quadro+2].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosDavid[quadro].position.x = 1058.391
		quadrosDavid[quadro].position.y = -2051.065
		quadrosDavid[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosDavid[quadro-1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	

func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'David'
			quadro = 0
			active = false
			quadrosDavid[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosDavid[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
