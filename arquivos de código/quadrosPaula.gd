...
func _ready():
	...
	quadrosPaula = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Paula1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Paula2"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Paula3")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosPaula[quadro-1].position.x = 1058.391
	quadrosPaula[quadro-1].position.y = -2051.065
	quadrosPaula[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosPaula[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosPaula[quadro].position.x = 1058.391
		quadrosPaula[quadro].position.y = -2051.065
		quadrosPaula[quadro+2].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosPaula[quadro+2].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosPaula[quadro].position.x = 1058.391
		quadrosPaula[quadro].position.y = -2051.065
		quadrosPaula[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosPaula[quadro-1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	

func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'Paula'
			quadro = 0
			active = false
			quadrosPaula[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosPaula[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
