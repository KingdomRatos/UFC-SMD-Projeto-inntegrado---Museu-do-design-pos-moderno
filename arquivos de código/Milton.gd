...
func _ready():
	...
	quadrosMilton = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 50").get_node("Milton1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 50").get_node("Milton2"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 50").get_node("Milton3")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosMilton[quadro-1].position.x = 1058.391
	quadrosMilton[quadro-1].position.y = -2051.065
	quadrosMilton[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosMilton[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosMilton[quadro].position.x = 1058.391
		quadrosMilton[quadro].position.y = -2051.065
		quadrosMilton[quadro+2].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosMilton[quadro+2].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosMilton[quadro].position.x = 1058.391
		quadrosMilton[quadro].position.y = -2051.065
		quadrosMilton[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosMilton[quadro-1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	

func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'Milton'
			quadro = 0
			quadrosMilton[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosMilton[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
