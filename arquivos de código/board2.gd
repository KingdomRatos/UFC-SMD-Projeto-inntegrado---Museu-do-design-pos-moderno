...
func _on_Tile_pressed(number):
	...
	if game_state == GAME_STATES.WON:
		get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").get_node("placa").get_node("I3").modulate = Color(1,1,1,1)
		get_tree().root.get_node("World").get_node("YSort").get_node("Player").podemover=true
		get_tree().root.get_node("World").adicionarPonto()
		visible = false
		return
