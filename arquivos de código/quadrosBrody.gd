...
func _ready():
	...
	quadrosBrody = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Brody1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Brody2"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Brody3")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosBrody[quadro-1].position.x = 1058.391
	quadrosBrody[quadro-1].position.y = -2051.065
	quadrosBrody[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosBrody[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosBrody[quadro].position.x = 1058.391
		quadrosBrody[quadro].position.y = -2051.065
		quadrosBrody[quadro+2].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosBrody[quadro+2].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosBrody[quadro].position.x = 1058.391
		quadrosBrody[quadro].position.y = -2051.065
		quadrosBrody[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosBrody[quadro-1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	

func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'Brody'
			quadro = 0
			active = false
			quadrosBrody[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosBrody[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
