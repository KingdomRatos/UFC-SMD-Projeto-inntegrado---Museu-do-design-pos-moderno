...
func _ready():
	...
	quadrosEd = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Ed1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Ed2"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 8090").get_node("Ed3")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosEd[quadro-1].position.x = 1058.391
	quadrosEd[quadro-1].position.y = -2051.065
	quadrosEd[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosEd[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosEd[quadro].position.x = 1058.391
		quadrosEd[quadro].position.y = -2051.065
		quadrosEd[quadro+2].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosEd[quadro+2].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosEd[quadro].position.x = 1058.391
		quadrosEd[quadro].position.y = -2051.065
		quadrosEd[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosEd[quadro-1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	

func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'Ed'
			quadro = 0
			active = false
			quadrosEd[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosEd[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
