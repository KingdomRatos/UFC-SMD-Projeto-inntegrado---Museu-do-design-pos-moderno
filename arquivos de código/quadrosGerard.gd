...
func _ready():
	...
	quadrosGerard = [get_tree().root.get_node("World").get_node("quadros").get_node("anos 701").get_node("Gerard1"), get_tree().root.get_node("World").get_node("quadros").get_node("anos 701").get_node("Gerard2")]

func _process(_delta):
	$interrogacao.visible = active
		
func proximoquadro():
	quadrosGerard[quadro-1].position.x = 1058.391
	quadrosGerard[quadro-1].position.y = -2051.065
	quadrosGerard[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
	quadrosGerard[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
func quadroanterior():
	if(quadro == 0):
		quadrosGerard[quadro].position.x = 1058.391
		quadrosGerard[quadro].position.y = -2051.065
		quadrosGerard[quadro+1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
		quadrosGerard[quadro+1].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
	else:
		quadrosGerard[quadro].position.x = 1058.391
		quadrosGerard[quadro].position.y = -2051.065
		quadrosGerard[quadro-1].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
...
func _input(event):
		if event.is_action_pressed("ui_select") and active:
			get_tree().root.get_node("World").get_node("CanvasLayer").get_node("UI").autor = 'Gerard'
			quadro = 0
			active = false
			quadrosGerard[quadro].position.x = get_tree().root.get_node("World").get_node("Camera2D").position.x
			quadrosGerard[quadro].position.y = get_tree().root.get_node("World").get_node("Camera2D").position.y
